...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // =========================
  // COACH TTS — NO MERCY MODE
  // =========================
  const coachTextEl = document.getElementById("coach-text");
  const coachTtsBtn = document.getElementById("btn-coach-tts");
  const coachTtsStatus = document.getElementById("coach-tts-status");

  if (coachTtsBtn) {
    coachTtsBtn.addEventListener("click", () => {
      if (!tts.supported) {
        showTtsUnavailable(coachTtsStatus);
        return;
      }
      const text = coachTextEl ? coachTextEl.innerText : "";
      if (!text.trim()) {
        if (coachTtsStatus) {
          coachTtsStatus.textContent = "Nothing to read. Coach text is empty.";
        }
        return;
      }
      speak(text);
      if (coachTtsStatus) {
        coachTtsStatus.textContent = "Reading coach instructions...";
      }
    });
  }

  if (!tts.supported && coachTtsStatus) {
    coachTtsStatus.textContent =
      "Voice mode is not available on this device or browser.";
  }
});
